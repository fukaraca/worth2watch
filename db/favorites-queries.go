...
func AddContentToFavorites(c *gin.Context, IMDB, cType string) error {
...
func GetFavoriteContents(c *gin.Context, page, items int) (*[]model.Movie, *[]model.Series, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	offset := (page - 1) * items
	username, _ := c.Cookie("uid")
...
func SearchFavorites(c *gin.Context, name string, genres []string, page, items int) (*[]model.Movie, *[]model.Series, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	offset := (page - 1) * items
	username, _ := c.Cookie("uid")
...
`, username, name, offset, items)
	...
	tempMovies, tempSeries, err := GetFavoriteContents(c, page, items)
	if err != nil {
		return nil, nil, err
	}
