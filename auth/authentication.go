...
func CheckCookie(c *gin.Context, toBeChecked, userId string) bool {
	...
	defer cancel()
	cookieVal, err := db.Cache.Do(ctx, "GET", toBeChecked).Result()
...
func CreateSession(username string, c *gin.Context) {
	...
	if ok, err := db.IsAdmin(c, username); ok && err == nil {
		db.Cache.Do(ctx, "SETEX", "admin-"+username, "3600", true)
	}

	db.Cache.Do(ctx, "SETEX", sessionToken.String(), "3600", username)
...
func DeleteSession(c *gin.Context) (bool, error) {
	...
	ctx, cancel := context.WithTimeout(c.Request.Context(), model.TIMEOUT)
	defer cancel()
	res, err := db.Cache.Do(ctx, "DEL", toBeChecked).Result()
	if err != nil {
		log.Println("session deletion err:", err)
		return false, fmt.Errorf("session deletion err:%s", err.Error())
	}
	if CheckAdminForLoggedIn(c, username) {
		res, err = db.Cache.Do(ctx, "DEL", "admin-"+username).Result()
		if err != nil {
...
func CheckAdminForLoggedIn(c *gin.Context, username string) bool {
	ctx, cancel := context.WithTimeout(c.Request.Context(), model.TIMEOUT)
	defer cancel()
