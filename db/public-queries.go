...
func GetThisMovieFromDB(c *gin.Context, id string) (*model.Movie, error) {
...
func GetThisSeriesFromDB(c *gin.Context, id string) (*model.Series, *[]model.Seasons, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	//get series
...
func GetEpisodesForaSeasonFromDB(c *gin.Context, seriesID, sN string) (*[]model.Episodes, error) {
...
func GetMoviesListWithPage(c *gin.Context, page, items int) (*[]model.Movie, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	offset := (page - 1) * items
...
func GetSeriesListWithPage(c *gin.Context, page, items int) (*[]model.Series, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	offset := (page - 1) * items
...
func SearchContent(c *gin.Context, name string, genres []string, page, items int) (*[]model.Movie, *[]model.Series, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	offset := (page - 1) * items
	...
	tempMovies, _ := GetMoviesListWithPage(c, page, items)
	tempSeries, _ := GetSeriesListWithPage(c, page, items)

	return tempMovies, tempSeries, nil
}

//FindSimilarContent queries for similar content and returns a specific amount of item from DB.
//Similarity is determined according to popularity
func FindSimilarContent(c *gin.Context, id, cType string) (*[]model.Movie, *[]model.Series, error) {
	ctx, cancel := context.WithTimeout(c.Request.Context(), TIMEOUT)
	defer cancel()
	amount := 2
	if cType == "movie" {
